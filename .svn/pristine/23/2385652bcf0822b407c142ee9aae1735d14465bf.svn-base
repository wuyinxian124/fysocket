...
//		System.out.println(""+ByteBufferSwap.byteBufferToString(msg));
	...
	public void decodeFine(ByteBuffer msg) {
		...
		if(!hsStatus){
			try {
				// 如果是握手返回信息
				System.out.println("握手返回信息收到,大小："+msg.limit());
			...
			// 用户验证信息返回
			try {
				System.out.println("用户验证返回信息收到，大小："+msg.limit()+ ",内容为："+ByteBufferSwap.byteBufferToString(msg));
			...
			// 普通消息
			System.out.println("收到消息包，大小："+msg.limit()+ ",内容为："+ByteBufferSwap.byteBufferToString(msg));
			readMsgT(msg);
		}
